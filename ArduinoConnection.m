...
% ------------------------------------------------------------------------------------
        ...
        function initializeUML(obj)
            ...
            par.x0 = 250;    % the initial signal strength
            par.x_lim = [0 500];   % the limits to the signal strength

            par.alpha = struct(...
                'limits',[0 1000],...       %range of the parameter space for alpha
                'N',51,...                %number of alpha values. If this value is set to 1, then the first element of alpha.limits would be the assumed alpha and the alpha parameter is not estimated.
            ...
            par.beta = struct(...
                'limits',[.001 1],...      %range of the parameter space for beta
            ...
            par.gamma  = struct(...
                'limits',[0 0.5],...      %range of the parameter space for gamma
                'N',51,...               %number of gamma values. If this value is set to 1, then the first element of gamma.limits would be the assumed gamma and the gamma parameter is not estimated.
                'scale','lin',...         %the linear or log spacing. Choose between 'lin' and 'log'.
            ...
            UMLobj = UML(par); % if UML object has not been created yet
            assignin("base","UMLobj",UMLobj);
        end
        function findNewSNR(obj)
            try
                UMLobj = evalin("base","UMLobj");
            catch
                obj.initializeUML();
                UMLobj = evalin("base","UMLobj");
            end
            S2VOLind = find(strcmp(obj.ParamNames,'S2VOL_PERMILLE'));
            resultCode = obj.Trials(end).Code;
            r = resultCode == 1;
            UMLobj.update(r);
            xnext = round(UMLobj.xnext); %note stimulation is not happening exactly where ML-PEST specifies, hopefully still nearby enough
            obj.UpdateParams_AddToQueue(S2VOLind,xnext);
            phi = UMLobj.phi(end,:);
            assignin("base","UMLobj",UMLobj);
            ...
            cla
            hold on
            plot(UMLobj.x,UMLobj.r,'o')
            xmax = 2 * phi(1);
            xplot = linspace(0,xmax,1000);
            
            yplot = UMLobj.psycfun(xplot,phi(1),phi(2),phi(3),phi(4));
            ylim([0 1])
            plot(xplot,yplot)   
        end
        ...
        function OnStateChanged(obj, ~, ~)
            ...
            if obj.StateCanUpdateParams(obj.State)  
                calibrationInd = find(strcmp(obj.ParamNames,'CALIBRATION'));
                if ~isempty(obj.Trials)
                if (obj.Trials(end).Code <= 2) % if result code indicated hit or miss
                    if obj.ParamValues(calibrationInd) % if in calibration mode
                        obj.findNewSNR()
                    end
                end
                end
